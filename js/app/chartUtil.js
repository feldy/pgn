...
js.app.chartUtil.chart3 = function () {
    var data1 = [null, 9200, 11811, 12000, 7662, 13887, 3000, 12000, 17733,
        9876, 7777, 9299
    ];
    var data2 = [3000, 12000, 17733, 9876, 12783, 12899, 10000, 9200, 11811,
        12000, 9898, 9349
    ];
    var data3 = [10000, 9200, 11811, 12000, 7662, 13887, 17733, 9876, 12783,
        12899, 10000, 3669
    ].reverse();
...
js.app.chartUtil.chart4 = function () {
    // Define the data
    var chartData = [10000, 9200, 11811, 12000, 7662, 13887, 14200, 12222,
        12000, 10009, 11288, 12099
    ];
    var chartData2 = [3000, 12000, 17733, 9876, 12783, 12899, 11811, 12000,
        7662, 13887, 14200, 12222
    ];
...
js.app.chartUtil.content1 = function () {
    ...
    var anim_c = new dc.action2d.Tooltip(pieChart, "default");
    var mag = new dojox.charting.action2d.MoveSlice(pieChart, "default");
    dojo.connect(dijit.byId("bulanExist"), "onChange", this, function (
        value) {
        dojo.xhrGet({
    ...
    var periode = js.app.eventUtil.getNamaBulan(nowDate[0]) + " " + nowDate[
        2];
    dojo.xhrGet({
        url: 'system/generate_report_chart.php?content=chart3&periode=' + periode,
        load: function (data) {
            var pars = JSON.parse(data);
            if (pars.length > 0) {
                pieChart.addSeries("Feldy Series", pars);
                pieChart.render();
                var legend = dijit.byId('legendChart1');
                if (legend) {
...
js.app.chartUtil.content2 = function () {
    ...
    var legend = null;
    var dc = dojox.charting;
    pieChart = new dc.Chart("chartNodeContent2");
    // pieChart.setTheme(dojox.charting.themes.PlotKit.green);
    pieChart.setTheme(dojox.charting.themes.MiamiNice);
    pieChart.addPlot("default", {
        type: "Pie",
    ...
    var anim_c = new dc.action2d.Tooltip(pieChart, "default");
    var mag = new dojox.charting.action2d.MoveSlice(pieChart, "default");
    dojo.connect(dijit.byId("bulanExist2"), "onChange", this, function (
        value) {
        dojo.xhrGet({
            url: 'system/generate_report_chart.php?content=chart4&periode=' + value,
            load: function (data) {
                var pars = JSON.parse(data);
                if (pars.length > 0) {
                    pieChart.addSeries(
    ...
    var periode = js.app.eventUtil.getNamaBulan(nowDate[0]) + " " + nowDate[
        2];
    dojo.xhrGet({
        url: 'system/generate_report_chart.php?content=chart4&periode=' + periode,
        load: function (data) {
            var pars = JSON.parse(data);
            if (pars.length > 0) {
                pieChart.addSeries("Feldy Series", pars);
                pieChart.render();
                var legend = dijit.byId('legendChart2');
                if (legend) {
                    legend.destroyRecursive(true);
                }
                dojox.charting.widget.Legend({
...
js.app.chartUtil.content3 = function () {
    ...
    var legend = null;
    var dc = dojox.charting;
    pieChart = new dc.Chart("chartNodeContent3");
    // pieChart.setTheme(dojox.charting.themes.PlotKit.green);
    pieChart.setTheme(dojox.charting.themes.MiamiNice);
    pieChart.addPlot("default", {
        type: "Pie",
    ...
    var anim_c = new dc.action2d.Tooltip(pieChart, "default");
    var mag = new dojox.charting.action2d.MoveSlice(pieChart, "default");
    dojo.connect(dijit.byId("bulanExist3"), "onChange", this, function (
        value) {
        dojo.xhrGet({
            url: 'system/generate_report_chart.php?content=chart5&periode=' + value,
            load: function (data) {
                var pars = JSON.parse(data);
                if (pars.length > 0) {
                    pieChart.addSeries(
    ...
    var periode = js.app.eventUtil.getNamaBulan(nowDate[0]) + " " + nowDate[
        2];
    dojo.xhrGet({
        url: 'system/generate_report_chart.php?content=chart5&periode=' + periode,
        load: function (data) {
            var pars = JSON.parse(data);
            if (pars.length > 0) {
                pieChart.addSeries("Feldy Series", pars);
                pieChart.render();
                var legend = dijit.byId('legendChart3');
                if (legend) {
                    legend.destroyRecursive(true);
                }
                dojox.charting.widget.Legend({
...
js.app.chartUtil.content4 = function () {
    dojo.xhrGet({
        ...
        load: function (data) {
            var pars = JSON.parse(data);
            if (pars.length > 0) {
                var chart = new dojox.charting.Chart2D(
                    "chartNodeContent4");
                chart.setTheme(dojox.charting.themes.MiamiNice);
                // chart.setTheme(dojox.charting.theme.Claro);
                chart.addPlot("default", {
...
js.app.chartUtil.content5 = function () {
    ...
    dojo.connect(dijit.byId("yearExists"), "onChange", this, function (
        value) {
        dojo.xhrGet({
    ...
    dojo.xhrGet({
        url: 'system/generate_report_chart.php?content=chart1&tahun=' + new Date()
            .getFullYear(),
        load: function (data) {
            var pars = JSON.parse(data);
            if (pars.length > 0) {
